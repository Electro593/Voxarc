...
MakeChunk(heap *Heap, region *Region, mesh *Mesh, v3s32 ChunkPos, u32 *TextureBytes)
{
	...
	for (u32 Y = 0; Y < 4; Y++) {
		...
		for (u32 Z = 0; Z < ChunkDims.Z; Z++)
			for (u32 X = 0; X < ChunkDims.X; X++) Blocks[INDEX_3D(X, Y, Z, ChunkDims.X, ChunkDims.Y)] = Type;
	}
		...
		for (u32 Y = 0; Y < C.Y; Y++) {
			for (u32 X = 0; X < C.X; X++) {
				for (u32 F = 0; F < 6; F++) {
					...
					if ((Consumed[I] & Flag) || T == BLOCK_NONE || (P.E[S] != B && Blocks[DI] != BLOCK_NONE))
						continue;
					...
					*Vertex++
						= (pt_vertex) {Mesh_EncodePosition(P0), RepV | RepU | TextureBytes[T] | R[K[F] + 0]};
					*Vertex++
						= (pt_vertex) {Mesh_EncodePosition(P1), RepV | RepU | TextureBytes[T] | R[K[F] + 1]};
					*Vertex++
						= (pt_vertex) {Mesh_EncodePosition(P2), RepV | RepU | TextureBytes[T] | R[K[F] + 2]};
					*Vertex++
						= (pt_vertex) {Mesh_EncodePosition(P3), RepV | RepU | TextureBytes[T] | R[K[F] + 3]};
...
internal b08
CollidesWithBlock(chunk Chunk, v3u32 BlockPos, v3r32 PosInChunk, v3r32 PlayerSize)
{
...
internal void
MakeNonVoxelChunk(heap *Heap, random *Random, region *Region, mesh *Mesh, v3s32 ChunkPos, u32 *TextureBytes)
{
	chunk *Chunk = AddChunkToRegion(Region, ChunkPos);
