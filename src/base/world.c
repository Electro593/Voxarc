...
GetChunk(region *Region, v3s32 Pos, u32 *ChunkIndexOut)
{
	...
	while (TRUE) {
		...
		if (Nodes[PrevNode].Children[J] == 0) return FALSE;
	...
	while (TRUE) {
		...
		if (Dims.X > 1 && Dims.Y > 1 && Dims.Z > 1) {
			if (Nodes[PrevNode].Children[J] == 0) {
				...
				Heap_Resize(
					Region->Nodes,
					Region->Nodes->Size + sizeof(region_node)
				);
...
)
{
	...
	Chunk->Pos = ChunkPos;
	Chunk->Blocks =
		Heap_Allocate(Heap, V3u32_Volume(ChunkDims) * sizeof(block_type));
	...
	u32 VertexIndex = 0;
	u32 VertexCount = 12;
	Chunk->Object.Vertices =
		Heap_Allocate(Heap, VertexCount * 4 * Mesh->VertexSize);
				...
				for (u32 F = 0; F < 6; F++) {
					...
					for (; P.E[S2] < C.E[S2]; P.E[S2]++) {
						...
						)
						{
							...
							// it's a different type, or it's being covered,
							// don't merge it.
							if ((Consumed[I] & Flag)
								|| Blocks[I] != T
								|| (P.E[S] != B && Blocks[DI] != BLOCK_NONE))
					...
					if (VertexIndex == VertexCount) {
						Heap_Resize(
							Chunk->Object.Vertices,
							(VertexCount + 12) * 4 * Mesh->VertexSize
						);
						Heap_Resize(
							Chunk->Object.Indices,
							(VertexCount + 12) * 6 * sizeof(u32)
						);
	...
	Chunk->Object.TranslationMatrix =
		M4x4r32_Translation(ChunkPos.X * 16, ChunkPos.Y * 16, ChunkPos.Z * 16);
...
)
{
	...
	Chunk->Object.Vertices =
		Heap_Allocate(Heap, VertexCount * Mesh->VertexSize);
	Chunk->Object.Indices =
		Heap_Allocate(Heap, TriangleCount * 3 * sizeof(u32));
	...
	for (u32 Z = 0; Z < VertexCountZ; Z++) {
		for (u32 X = 0; X < VertexCountX; X++) {
			v3r32 Pos = {
				2.0f * X / SquareCountX
					- 1
					+ R32_RandRange(Random, -0.01, 0.01),
	...
	for (u32 Z = 0; Z < VertexCountZ; Z++) {
		for (u32 X = 0; X < VertexCountX; X++) {
			...
			v3r32 Sum = V3r32_Add(
				V3r32_Add(V3r32_Add(TL, TM), V3r32_Add(TR, BL)),
				V3r32_Add(BM, BR)
			);
			v3r32 Normal = V3r32_Norm(Sum);

			Vertices[INDEX_2D(X, Z, VertexCountX)].Normal =
				Mesh_EncodeNormal(Normal);
		}
	}
