...
Update(platform_state *Platform)
{
	...
	if (Platform->CursorIsDisabled) {
		...
		if (Game->Flying) {
			...
			// Walking
			r32 DeltaFront
				= (!!Platform->Keys[ScanCode_W] - !!Platform->Keys[ScanCode_S]) * WalkForce * Game->Mass;
			r32 DeltaRight
				= (!!Platform->Keys[ScanCode_D] - !!Platform->Keys[ScanCode_A]) * WalkForce * Game->Mass;
	...
	if ((Platform->Updates & WINDOW_RESIZED) || Moved || !V3r32_IsEqual(DirDelta, (v3r32) {0})) {
		...
		Renderer->ViewMatrix = (m4x4r32) {Right.X,
										  Right.Y,
										  Right.Z,
										  V3r32_Dot(Right, Pos),
										  Up.X,
	...
	if (Platform->CursorIsDisabled) {
		...
		if (Game->AimBlockValid) {
			...
			*Matrix			= M4x4r32_Mul(M4x4r32_Mul(Translation, Rotation), Scaling);
			...
			s32 *VertexCount
				= (s32 *) Renderer->Shaders[ShaderID_P3].Mesh.VertexCounts->Data + Game->AimBlockObjectIndex;
			*VertexCount = 24;
