...
WriteFiles(gen_data Data)
{
    ...
    FILE *VAA = fopen(GLYPHS_ATLAS, "wb");
    if(!VAA)
    {
        return;
    }
...
main()
{
    ...
    for(u32 Glyph = 'A';
        ...
        ++Glyph)
    {
        BeginGlyphAsset(&Data, "C:/Windows/Fonts/cour.ttf", Glyph);
        AddTag_U32(&Data, AssetTag_Codepoint, Glyph);
        EndAsset(&Data);
    }
    EndType(&Data);
