...
Str_GetHeap(void)
{
    return __Global->StrHeap;
}
internal void
Str_SetHeap(heap *Heap)
{
    __Global->StrHeap = Heap;
}
...
{
    return &__Global->StrHeapDebugData;
}

internal void
_Str_HeapMark_DEBUG(void)
{
    Heap_Mark_DEBUG(__Global->StrHeap, &__Global->StrHeapDebugData);
}

internal void
_Str_HeapCheck_DEBUG(void)
{
    Heap_Check_DEBUG(__Global->StrHeap, &__Global->StrHeapDebugData);
}

internal void
_Str_HeapPrint_DEBUG(void)
{
    Heap_Print_DEBUG(__Global->StrHeap, &__Global->StrHeapDebugData);
}
#endif
           ...
           u32 Length)
{
    ...
    str Str = Heap_Allocate(__Global->StrHeap, Length + 1 + sizeof(str_footer));
