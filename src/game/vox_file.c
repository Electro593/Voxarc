    ...
    while(*TextByte != '\0')
    {
        ...
        if(*TextByte == '\n')
        {
            if(!Commented)
            {
                ...
                {
                    Config->VisionBitsPerChannel = Str_ToU32(Buffer);
                }
                else if(Str_Cmp(TypeName, "VisionChannelBinary") == 0)
                {
                    Config->VisionChannelBinary = Str_ToU32(Buffer);
                }
                else if(Str_Cmp(TypeName, "FOV") == 0)
                {
                    Config->FOV = Str_ToU32(Buffer);
                }
                else if(Str_Cmp(TypeName, "Sensitivity") == 0)
                {
                    Config->Sensitivity = Str_ToR32(Buffer);
                }
                else if(Str_Cmp(TypeName, "RenderDistance") == 0)
                {
                    Config->RenderDistance = V3u32_1(Str_ToU32(Buffer));
                }
                else if(Str_Cmp(TypeName, "MoveLeft") == 0)
                {
                    Config->MoveLeft = Str_ToU32(Buffer);
                }
                else if(Str_Cmp(TypeName, "MoveForward") == 0)
                {
                    Config->MoveForward = Str_ToU32(Buffer);
                }
                else if(Str_Cmp(TypeName, "MoveRight") == 0)
                {
                    Config->MoveRight = Str_ToU32(Buffer);
                }
                else if(Str_Cmp(TypeName, "MoveBack") == 0)
                {
                    Config->MoveBack = Str_ToU32(Buffer);
                }
                else if(Str_Cmp(TypeName, "MoveUp") == 0)
                {
                    Config->MoveUp = Str_ToU32(Buffer);
                }
                else if(Str_Cmp(TypeName, "MoveDown") == 0)
                {
                    Config->MoveDown = Str_ToU32(Buffer);
                }
            }
            else
            {
...
File_CreateAssetPack(heap *Heap)
{
    ...
    u32 AssetGroupID = Str_ToU32(Str_Sub(&TempStr, FileText, StartIndex, EndIndex)); Str_Free(TempStr);
                    ...
                    u32 ExtraOffset)
{
    ...
    switch(AssetTagTypes[Tag->ID])
    {
        ...
        case AssetTagType_R32:
        {
            Tag->Value.R32 = Str_ToR32(ValueStr);
        } break;
